...
USER=$(whoami)
HOSTNAME=$(hostname)
IP_ADDRESS=$(hostname -I | awk '{print $1}')  # 服务器内网 IP
...
# 生成消息内容
MESSAGE="🔔 SSH 登录通知%0A👤 用户: $USER%0A🖥 服务器: $HOSTNAME%0A🌐 内网IP:$IP_ADDRESS%0A🌐 公网IP: $SERVER_PUBLIC_IP%0A📡 登录IP: $SSH_CLIENT_IP%0A📍 位置: $LOCATION"
# 发送到 Telegram
